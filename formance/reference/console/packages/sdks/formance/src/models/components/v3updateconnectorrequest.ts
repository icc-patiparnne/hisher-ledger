/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import {
    V3AdyenConfig,
    V3AdyenConfig$inboundSchema,
    V3AdyenConfig$Outbound,
    V3AdyenConfig$outboundSchema,
} from "./v3adyenconfig.js";
import {
    V3AtlarConfig,
    V3AtlarConfig$inboundSchema,
    V3AtlarConfig$Outbound,
    V3AtlarConfig$outboundSchema,
} from "./v3atlarconfig.js";
import {
    V3BankingcircleConfig,
    V3BankingcircleConfig$inboundSchema,
    V3BankingcircleConfig$Outbound,
    V3BankingcircleConfig$outboundSchema,
} from "./v3bankingcircleconfig.js";
import {
    V3ColumnConfig,
    V3ColumnConfig$inboundSchema,
    V3ColumnConfig$Outbound,
    V3ColumnConfig$outboundSchema,
} from "./v3columnconfig.js";
import {
    V3CurrencycloudConfig,
    V3CurrencycloudConfig$inboundSchema,
    V3CurrencycloudConfig$Outbound,
    V3CurrencycloudConfig$outboundSchema,
} from "./v3currencycloudconfig.js";
import {
    V3DummypayConfig,
    V3DummypayConfig$inboundSchema,
    V3DummypayConfig$Outbound,
    V3DummypayConfig$outboundSchema,
} from "./v3dummypayconfig.js";
import {
    V3GenericConfig,
    V3GenericConfig$inboundSchema,
    V3GenericConfig$Outbound,
    V3GenericConfig$outboundSchema,
} from "./v3genericconfig.js";
import {
    V3IncreaseConfig,
    V3IncreaseConfig$inboundSchema,
    V3IncreaseConfig$Outbound,
    V3IncreaseConfig$outboundSchema,
} from "./v3increaseconfig.js";
import {
    V3MangopayConfig,
    V3MangopayConfig$inboundSchema,
    V3MangopayConfig$Outbound,
    V3MangopayConfig$outboundSchema,
} from "./v3mangopayconfig.js";
import {
    V3ModulrConfig,
    V3ModulrConfig$inboundSchema,
    V3ModulrConfig$Outbound,
    V3ModulrConfig$outboundSchema,
} from "./v3modulrconfig.js";
import {
    V3MoneycorpConfig,
    V3MoneycorpConfig$inboundSchema,
    V3MoneycorpConfig$Outbound,
    V3MoneycorpConfig$outboundSchema,
} from "./v3moneycorpconfig.js";
import {
    V3PlaidConfig,
    V3PlaidConfig$inboundSchema,
    V3PlaidConfig$Outbound,
    V3PlaidConfig$outboundSchema,
} from "./v3plaidconfig.js";
import {
    V3PowensConfig,
    V3PowensConfig$inboundSchema,
    V3PowensConfig$Outbound,
    V3PowensConfig$outboundSchema,
} from "./v3powensconfig.js";
import {
    V3QontoConfig,
    V3QontoConfig$inboundSchema,
    V3QontoConfig$Outbound,
    V3QontoConfig$outboundSchema,
} from "./v3qontoconfig.js";
import {
    V3StripeConfig,
    V3StripeConfig$inboundSchema,
    V3StripeConfig$Outbound,
    V3StripeConfig$outboundSchema,
} from "./v3stripeconfig.js";
import {
    V3TinkConfig,
    V3TinkConfig$inboundSchema,
    V3TinkConfig$Outbound,
    V3TinkConfig$outboundSchema,
} from "./v3tinkconfig.js";
import {
    V3WiseConfig,
    V3WiseConfig$inboundSchema,
    V3WiseConfig$Outbound,
    V3WiseConfig$outboundSchema,
} from "./v3wiseconfig.js";
import * as z from "zod";

export type V3UpdateConnectorRequest =
    | (V3StripeConfig & { provider: "Stripe" })
    | (V3ColumnConfig & { provider: "Column" })
    | (V3GenericConfig & { provider: "Generic" })
    | (V3WiseConfig & { provider: "Wise" })
    | (V3AtlarConfig & { provider: "Atlar" })
    | (V3CurrencycloudConfig & { provider: "Currencycloud" })
    | (V3DummypayConfig & { provider: "Dummypay" })
    | (V3IncreaseConfig & { provider: "Increase" })
    | (V3MangopayConfig & { provider: "Mangopay" })
    | (V3ModulrConfig & { provider: "Modulr" })
    | (V3MoneycorpConfig & { provider: "Moneycorp" })
    | (V3PlaidConfig & { provider: "Plaid" })
    | (V3TinkConfig & { provider: "Tink" })
    | (V3QontoConfig & { provider: "Qonto" })
    | (V3AdyenConfig & { provider: "Adyen" })
    | (V3BankingcircleConfig & { provider: "Bankingcircle" })
    | (V3PowensConfig & { provider: "Powens" });

/** @internal */
export const V3UpdateConnectorRequest$inboundSchema: z.ZodType<
    V3UpdateConnectorRequest,
    z.ZodTypeDef,
    unknown
> = z.union([
    V3StripeConfig$inboundSchema.and(
        z.object({ provider: z.literal("Stripe") }).transform((v) => ({ provider: v.provider }))
    ),
    V3ColumnConfig$inboundSchema.and(
        z.object({ provider: z.literal("Column") }).transform((v) => ({ provider: v.provider }))
    ),
    V3GenericConfig$inboundSchema.and(
        z.object({ provider: z.literal("Generic") }).transform((v) => ({ provider: v.provider }))
    ),
    V3WiseConfig$inboundSchema.and(
        z.object({ provider: z.literal("Wise") }).transform((v) => ({ provider: v.provider }))
    ),
    V3AtlarConfig$inboundSchema.and(
        z.object({ provider: z.literal("Atlar") }).transform((v) => ({ provider: v.provider }))
    ),
    V3CurrencycloudConfig$inboundSchema.and(
        z
            .object({ provider: z.literal("Currencycloud") })
            .transform((v) => ({ provider: v.provider }))
    ),
    V3DummypayConfig$inboundSchema.and(
        z.object({ provider: z.literal("Dummypay") }).transform((v) => ({ provider: v.provider }))
    ),
    V3IncreaseConfig$inboundSchema.and(
        z.object({ provider: z.literal("Increase") }).transform((v) => ({ provider: v.provider }))
    ),
    V3MangopayConfig$inboundSchema.and(
        z.object({ provider: z.literal("Mangopay") }).transform((v) => ({ provider: v.provider }))
    ),
    V3ModulrConfig$inboundSchema.and(
        z.object({ provider: z.literal("Modulr") }).transform((v) => ({ provider: v.provider }))
    ),
    V3MoneycorpConfig$inboundSchema.and(
        z.object({ provider: z.literal("Moneycorp") }).transform((v) => ({ provider: v.provider }))
    ),
    V3PlaidConfig$inboundSchema.and(
        z.object({ provider: z.literal("Plaid") }).transform((v) => ({ provider: v.provider }))
    ),
    V3TinkConfig$inboundSchema.and(
        z.object({ provider: z.literal("Tink") }).transform((v) => ({ provider: v.provider }))
    ),
    V3QontoConfig$inboundSchema.and(
        z.object({ provider: z.literal("Qonto") }).transform((v) => ({ provider: v.provider }))
    ),
    V3AdyenConfig$inboundSchema.and(
        z.object({ provider: z.literal("Adyen") }).transform((v) => ({ provider: v.provider }))
    ),
    V3BankingcircleConfig$inboundSchema.and(
        z
            .object({ provider: z.literal("Bankingcircle") })
            .transform((v) => ({ provider: v.provider }))
    ),
    V3PowensConfig$inboundSchema.and(
        z.object({ provider: z.literal("Powens") }).transform((v) => ({ provider: v.provider }))
    ),
]);

/** @internal */
export type V3UpdateConnectorRequest$Outbound =
    | (V3StripeConfig$Outbound & { provider: "Stripe" })
    | (V3ColumnConfig$Outbound & { provider: "Column" })
    | (V3GenericConfig$Outbound & { provider: "Generic" })
    | (V3WiseConfig$Outbound & { provider: "Wise" })
    | (V3AtlarConfig$Outbound & { provider: "Atlar" })
    | (V3CurrencycloudConfig$Outbound & { provider: "Currencycloud" })
    | (V3DummypayConfig$Outbound & { provider: "Dummypay" })
    | (V3IncreaseConfig$Outbound & { provider: "Increase" })
    | (V3MangopayConfig$Outbound & { provider: "Mangopay" })
    | (V3ModulrConfig$Outbound & { provider: "Modulr" })
    | (V3MoneycorpConfig$Outbound & { provider: "Moneycorp" })
    | (V3PlaidConfig$Outbound & { provider: "Plaid" })
    | (V3TinkConfig$Outbound & { provider: "Tink" })
    | (V3QontoConfig$Outbound & { provider: "Qonto" })
    | (V3AdyenConfig$Outbound & { provider: "Adyen" })
    | (V3BankingcircleConfig$Outbound & { provider: "Bankingcircle" })
    | (V3PowensConfig$Outbound & { provider: "Powens" });

/** @internal */
export const V3UpdateConnectorRequest$outboundSchema: z.ZodType<
    V3UpdateConnectorRequest$Outbound,
    z.ZodTypeDef,
    V3UpdateConnectorRequest
> = z.union([
    V3StripeConfig$outboundSchema.and(
        z.object({ provider: z.literal("Stripe") }).transform((v) => ({ provider: v.provider }))
    ),
    V3ColumnConfig$outboundSchema.and(
        z.object({ provider: z.literal("Column") }).transform((v) => ({ provider: v.provider }))
    ),
    V3GenericConfig$outboundSchema.and(
        z.object({ provider: z.literal("Generic") }).transform((v) => ({ provider: v.provider }))
    ),
    V3WiseConfig$outboundSchema.and(
        z.object({ provider: z.literal("Wise") }).transform((v) => ({ provider: v.provider }))
    ),
    V3AtlarConfig$outboundSchema.and(
        z.object({ provider: z.literal("Atlar") }).transform((v) => ({ provider: v.provider }))
    ),
    V3CurrencycloudConfig$outboundSchema.and(
        z
            .object({ provider: z.literal("Currencycloud") })
            .transform((v) => ({ provider: v.provider }))
    ),
    V3DummypayConfig$outboundSchema.and(
        z.object({ provider: z.literal("Dummypay") }).transform((v) => ({ provider: v.provider }))
    ),
    V3IncreaseConfig$outboundSchema.and(
        z.object({ provider: z.literal("Increase") }).transform((v) => ({ provider: v.provider }))
    ),
    V3MangopayConfig$outboundSchema.and(
        z.object({ provider: z.literal("Mangopay") }).transform((v) => ({ provider: v.provider }))
    ),
    V3ModulrConfig$outboundSchema.and(
        z.object({ provider: z.literal("Modulr") }).transform((v) => ({ provider: v.provider }))
    ),
    V3MoneycorpConfig$outboundSchema.and(
        z.object({ provider: z.literal("Moneycorp") }).transform((v) => ({ provider: v.provider }))
    ),
    V3PlaidConfig$outboundSchema.and(
        z.object({ provider: z.literal("Plaid") }).transform((v) => ({ provider: v.provider }))
    ),
    V3TinkConfig$outboundSchema.and(
        z.object({ provider: z.literal("Tink") }).transform((v) => ({ provider: v.provider }))
    ),
    V3QontoConfig$outboundSchema.and(
        z.object({ provider: z.literal("Qonto") }).transform((v) => ({ provider: v.provider }))
    ),
    V3AdyenConfig$outboundSchema.and(
        z.object({ provider: z.literal("Adyen") }).transform((v) => ({ provider: v.provider }))
    ),
    V3BankingcircleConfig$outboundSchema.and(
        z
            .object({ provider: z.literal("Bankingcircle") })
            .transform((v) => ({ provider: v.provider }))
    ),
    V3PowensConfig$outboundSchema.and(
        z.object({ provider: z.literal("Powens") }).transform((v) => ({ provider: v.provider }))
    ),
]);

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace V3UpdateConnectorRequest$ {
    /** @deprecated use `V3UpdateConnectorRequest$inboundSchema` instead. */
    export const inboundSchema = V3UpdateConnectorRequest$inboundSchema;
    /** @deprecated use `V3UpdateConnectorRequest$outboundSchema` instead. */
    export const outboundSchema = V3UpdateConnectorRequest$outboundSchema;
    /** @deprecated use `V3UpdateConnectorRequest$Outbound` instead. */
    export type Outbound = V3UpdateConnectorRequest$Outbound;
}
