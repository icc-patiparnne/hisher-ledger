...
interface DataTableSkeletonProps extends React.HTMLAttributes<HTMLDivElement> {
   ...
   * When withColumnsOptions is true, this includes the header row.
   * When withColumnsOptions is false, this is just the body rows.
   * @default 10
  ...
  /**
   * Flag to show the columns options in the top right corner.
   * @default true
   * @type boolean | undefined
   */
  withColumnsOptions?: boolean;
    ...
    >
      ...
      <div className="rounded-md border border-border">
        <Table>
          ...
          <TableBody>
            {Array.from({
              length: withColumnsOptions ? Math.max(0, rowCount - 1) : rowCount,
            }).map((_, i) => (
              <TableRow key={i} className="hover:bg-transparent">
                {Array.from({ length: columnCount }).map((_, j) => (
                  <TableCell
